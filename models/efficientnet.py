...
class EdgeResidual(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, exp_kernel_size=3, exp_ratio=1.0, fake_in_chs=0,
        ...
        # Point-wise linear projection
        self.conv_pwl = select_conv2d(mid_chs, out_chs, pw_kernel_size, stride=stride, dilation=dilation, padding=pad_type)
        self.bn2 = norm_layer(out_chs, **norm_kwargs)
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, num_features=1280, in_chans=3, stem_size=32,
                 ...
                 pad_type='', act_layer=nn.ReLU, drop_rate=0.2, drop_connect_rate=0.2,
        ...
        builder = EfficientNetBuilder(channel_multiplier, channel_divisor, channel_min, 32, pad_type, act_layer, se_kwargs,
    ...
    def forward(self, x, epoch=0, i=0, acc=0):
        x = self.conv_stem(x)
        x = self.bn1(x)
        x = self.act1(x)
        x = self.blocks(x)
...
def _gen_efficientnet(variant, channel_multiplier=1.0, depth_multiplier=1.0):
    ...
    """
    arch_def = [['ds_r1_k3_s1_e1_c16_se0.25']]
    model_kwargs = dict(
        ...
        act_layer=Swish,
        norm_kwargs=None#resolve_bn_args(kwargs),
    )
...
#@register_model
def efficientnet_b0(parameters):
    global args
    args = parameters
    # NOTE for train, drop_rate should be 0.2, drop_connect_rate should be 0.2
    model = _gen_efficientnet('efficientnet_b0', channel_multiplier=1.0, depth_multiplier=1.0)
    return model
